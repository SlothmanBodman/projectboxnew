    ...
    <div class="content-container" id="{{$post->type}}">
        ...
        <div class="content-container-footer">
          <p class="paragraph">
            @if(in_array($post->id, $userLikes))
              <i class="fas fa-heart unlike-trigger" name="{{$post->id}}" value="{{$post->user_id}}" style="color: var(--primary);"></i>
              <i class="fas fa-heart like-trigger hidden" id="like{{$post->id}}"  name="{{$post->id}}" value="{{$post->user_id}}" style="color: #9b9b9b;cursor: pointer"></i>
              <span id="likesCount{{$post->id}}">{{$post->likes}}</span>
            @else
              <i class="fas fa-heart unlike-trigger hidden" id="unlike{{$post->id}}" name="{{$post->id}}" value="{{$post->user_id}}" style="color: var(--primary);"></i>
              <i class="fas fa-heart like-trigger" id="like{{$post->id}}" name="{{$post->id}}" value="{{$post->user_id}}" style="color: #9b9b9b;cursor: pointer"></i>
        ...
        <div class="content-container-footer" style="background-color: var(--bg-green);">
          <form class="" action="" method="post">
            ...
            <button type="button" name="{{$post->id}}" class="comment-trigger" value="{{$post->user_id}}" style="display:inline-block; width: 20%;">Comment</button>
          </form>
        </div>
        <!-- COMMENTS SECTION -->
        <div class="content-container-body">
